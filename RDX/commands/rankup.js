...
module.exports = {
  config: {
    ...
    countDown: 5,
    role: 0,
    description: "View your level progress and rankup card.",
    category: "system",
    guide: "{pn}",
    prefix: true
  },
  ...
  handleRankup: async function ({ api, event, Users, Currencies }) {
    ...
    try {
      ...
      // Calculate level: Each user has their own level based on their exp
      // Formula matches listen.js: Level = sqrt(exp/12.5), capped at max 10
      const rawLevel = Math.floor(Math.sqrt(userExp / 12.5));
      const currentLevel = Math.min(rawLevel, 10); // Max level 10
